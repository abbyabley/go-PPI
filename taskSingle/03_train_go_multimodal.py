...
class AdvancedFlexibleGOPredictor(nn.Module):
    ...
    def forward(self, esm, adj, ppi_prior, mask):
        ...
        global_rep = (h_encoded * mask_weights).sum(dim=1) / (mask_weights.sum(dim=1) + 1e-6) 
        
        if self.is_baseline:
            final_rep = global_rep
        else:
            raw_attn = self.attn_net(h_encoded)
            ppi_bias = ppi_prior.unsqueeze(-1) * 2.0
            ...
            local_rep = (h_encoded * attn_weights).sum(dim=1) 
            concat_rep = torch.cat([global_rep, local_rep], dim=-1)
