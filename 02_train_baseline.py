...
class StaticGODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            # 基线模型不需要加载 ppi，但我们为了 collate_fn 统一，还是加载一个 dummy 占位
            ppi = torch.zeros(esm.shape[0]).float()
        except: return None
        ...
        return {"esm": esm, "ppi_prior": ppi, "go_labels": go_labels, "mask": torch.ones(esm.shape[0])}
...
class BaselineGOPredictor(nn.Module):
    ...
    def forward(self, esm, ppi_prior, mask):
        ...
        # 🟢 Baseline：完全屏蔽 PPI，使用全局均值池化
        mask_weights = mask.unsqueeze(-1)
        pooled_rep = (h_esm * mask_weights).sum(dim=1) / (mask_weights.sum(dim=1) + 1e-6) 
        return self.classifier(pooled_rep)

def train_baseline():
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(">>> 正在训练 Baseline (仅序列) 模型...")

    train_ds = StaticGODataset(f'{BASE_DIR}/go_train_cleaned.tsv', GO_MAP, NUM_GO_CLASSES)
    val_ds = StaticGODataset(f'{BASE_DIR}/go_val_cleaned.tsv', GO_MAP, NUM_GO_CLASSES)
    ...
    model = BaselineGOPredictor(num_classes=NUM_GO_CLASSES).to(device)
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"Epoch {epoch+1} | Baseline AUPRC: {auprc:.4f}")
        if auprc > best_auprc:
            best_auprc = auprc
            torch.save(model.state_dict(), "best_go_baseline.pth")
